...
## 프로젝트 개요
4개 채용 플랫폼(원티드, 사람인, 잡코리아, 리멤버)에서 
데이터 분석가 채용공고를 매일 자동 수집·정제·분류·알림하는 
End-to-End 파이프라인입니다.
...
| 플랫폼 | 문제 | 해결 |
|--------|------|------|
| 원티드 | React SPA로 HTML 스크래핑 불가 | 네트워크 탭 분석으로 JSON API 발견 |
| 사람인 | Playwright 봇 탐지 타임아웃 | 모바일 API → PC API 전환 |
| 잡코리아 | UI 요소가 회사명 클래스 공유 | INVALID_COMPANIES 필터링 |
| 잡코리아 | 급여 없는 공고 파싱 밀림 | 인덱스 기반 → 키워드 기반 파싱 |
| n8n | Clear sheet 420번 반복 실행 | Clear 노드 맨 앞으로 이동 |

## 한계점 및 개선 방향
| 한계점 | 개선 방향 |
|--------|----------|
| 사람인 description 수집 불가 | 공식 API 승인 후 수집 예정 |
| 로컬 환경 의존 (절전 모드 시 미실행) | GCP 클라우드 서버 이전 예정 |
| 키워드 기반 산업군 분류 오류 가능 | LLM 기반 분류로 개선 검토 |
